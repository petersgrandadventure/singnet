...
class NodeLookup(object):
    ...
    def __init__(self, label_lookup_path=None, uid_lookup_path=None):
        ...
        model_directory = os.path.join(AGENT_DIRECTORY, 'model_data')

        label_lookup_path = os.path.join(model_directory, 'imagenet_2012_challenge_label_map_proto.pbtxt')
        if not tf.gfile.Exists(label_lookup_path):
            tf.logging.fatal('Missing Label file %s', label_lookup_path)

        uid_lookup_path = os.path.join(model_directory, 'imagenet_synset_to_human_label_map.txt')
