...
class Shortlist(object):
    ...
    def complete(self):
        ...
        logger.debug('Completion value is: %s', self.completion_value)
    ...
    def get_next_iteration(self, alpha):
        ...
        logger.debug('Getting next iteration alpha: %s', alpha)
        ...
        with self.lock:
            for node, completed in self.list:
                logger.debug('Getting next iteration %s, %s', node, completed)
                if not completed:
